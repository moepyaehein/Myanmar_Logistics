...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string}>}) {
  ...
  return <><Link href="/admin/shipments" className="back-link">← All shipments</Link><div className="page-heading"><div><p className="eyebrow">SHIPMENT MANAGEMENT</p><h1>{shipment.shipment_number}</h1><p className="muted">{shipment.origin} → {shipment.destination}</p></div><StatusBadge status={shipment.status} /></div>{saved==="1" && <p className="success-notice" role="status">Shipment updated.</p>}
    <section className="panel"><div className="panel-heading"><h2>Shipment details</h2>{gate && <StatusBadge status={gate.status} />}</div><dl className="shipment-facts">{facts.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className="map-caption">Last updated {formatDate(shipment.updated_at,true)}{gate?.reason && ` · Gate note: ${gate.reason}`}</p></section>
