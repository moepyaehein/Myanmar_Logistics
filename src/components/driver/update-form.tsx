...
export function DriverUpdateForm(props:{shipment:string;revision:string;status:ShipmentStatus;requestId:string;occurredAt:string;userId:string;shipmentNumber:string}) {
  ...
  function useMyLocation() {
    ...
    try {
      navigator.geolocation.getCurrentPosition(position=>{
        ...
        setLocationMessage(`Your device location is ready${Number.isFinite(accuracy)&&accuracy>=0 ? ` (estimated accuracy: ${Math.ceil(accuracy)} m)` : ""}. Press Save shipment update to share it.`);
      },error=>{
        if(request!==locationRequest.current)return;
        setLocating(false);
  ...
  return <section className="panel"><div className="panel-heading"><div><h2>Update shipment</h2><p className="panel-subtitle">Status, note and location</p></div></div><form key={revision} action={action} onSubmit={handleSubmit} className="shipment-form driver-update-form" onInputCapture={capture} onChangeCapture={capture} onClickCapture={capture} onSubmitCapture={capture}>
    ...
    <fieldset disabled={pending}><legend>Progress update</legend><div className="form-field"><label htmlFor="driver-status">Status</label><select id="driver-status" name="status" defaultValue={status}>{choices.map(value=><option key={value} value={value}>{SHIPMENT_STATUS_LABELS[value]}{value===status ? " (current)" : ""}</option>)}</select></div><div className="form-field"><label htmlFor="driver-note">Status note</label><textarea id="driver-note" name="note" maxLength={2000} rows={3} placeholder="Checkpoint details, cargo condition or delivery note" /></div></fieldset>
    <fieldset disabled={pending}><legend>Location (optional)</legend><p className="form-help">Use your phone's location—no coordinates to type. Location is shared with shipment viewers only when you save this update.</p>
      <div className="gps-presets"><button type="button" className="button button-dark" onClick={useMyLocation} disabled={locating}>{locating ? "Finding my location…" : "Use my location"}</button>{locating&&<button type="button" className="button signout-button" onClick={()=>selectLocation("","","Location request cancelled. No new position will be shared.")}>Cancel location request</button>}<button type="button" className="button signout-button" onClick={()=>selectLocation("","","No new location selected. Your last saved location will be kept.")}>Clear location</button></div>
      <p role="status" aria-live="polite" className="location-status">{locationMessage}</p>
      <details className="location-options"><summary>Demo locations</summary><p className="form-help">Choose a simulated position for the assignment demo.</p><div className="gps-presets">{[["Yangon","16.8409","96.1735"],["Mandalay","21.9588","96.0891"],["Muse","23.9869","97.9040"],["Myawaddy","16.6891","98.5089"]].map(([name,lat,lng])=><button type="button" className="button signout-button" key={name} onClick={()=>selectLocation(lat,lng,`${name} demo location selected. Press Save shipment update to share it.`)}>{name}</button>)}</div></details>
      <details className="location-options"><summary>Advanced: view or edit coordinates</summary><div className="form-grid"><div className="form-field"><label htmlFor="latitude">Latitude</label><input id="latitude" name="latitude" type="number" step="any" min={-90} max={90} value={latitude} onChange={event=>selectLocation(event.target.value,longitude,"Manual coordinates selected. Enter both values before saving.")} /></div><div className="form-field"><label htmlFor="longitude">Longitude</label><input id="longitude" name="longitude" type="number" step="any" min={-180} max={180} value={longitude} onChange={event=>selectLocation(latitude,event.target.value,"Manual coordinates selected. Enter both values before saving.")} /></div></div><small>Blank coordinates preserve the previous location.</small></details><noscript>Enable JavaScript to use device or demo locations. Manual coordinates are available under Advanced.</noscript>
    </fieldset>
    {queuedMessage && <p className="success-notice" role="status">{queuedMessage}</p>}
    <p role="status" className={state.message ? "auth-error" : ""}>{state.message}</p>{state.message && <a href={`/driver/shipments/${shipment}`} className="text-link">Reload latest shipment</a>}<button type="submit" className="button button-dark driver-save" disabled={pending||locating}>{pending ? "Saving update…" : "Save shipment update"}</button>
  </form></section>;
}
